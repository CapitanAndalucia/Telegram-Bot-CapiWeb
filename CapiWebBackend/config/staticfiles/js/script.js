...
async function fetchTickets(reset=false) {
  ...
  document.getElementById("ticketCount").innerText = `${data.count} tickets`;
  hasMore = !!data.next;
...
function renderTicket(ticket) {
  ...
  card.innerHTML = `
    ...
    <div class="ticket-actions">
      <button class="action-btn edit-btn" onclick="openModal(${ticket.id}, '${escapeJs(ticket.titulo)}', '${ticket.coste}', '${fecha}')">✏️</button>
      <button class="action-btn delete-btn" onclick="deleteTicket(${ticket.id})">🗑️</button>
    </div>
  `;
